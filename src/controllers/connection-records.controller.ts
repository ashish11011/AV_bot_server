...
function validParams(req: Request, res: Response) {
  if (
    !Number.isSafeInteger(Number(req.params.tenantId)) ||
    Number(req.params.tenantId) <= 0 ||
...
export async function handleWhatsappNumber(req: Request, res: Response) {
  if (!validParams(req, res)) return;
  try {
    const [parent] = await db
      .select({ id: whatsappConnect.id })
    ...
    const condition = req.params.id
      ? and(scope, eq(whatsappConnectNumber.id, String(req.params.id)))
      : scope;
    if (req.method === 'GET') {
      const rows = await db
        .select()
        .from(whatsappConnectNumber)
        .where(condition)
        .orderBy(whatsappConnectNumber.createdAt);
      if (req.params.id && !rows[0]) return res.status(404).json({ msg: 'Record not found' });
    ...
    if (req.method === 'DELETE') {
      ...
      if (!rows.length) return res.status(404).json({ msg: 'Record not found' });
      return res.json({ data: null });
    }
    const { numberId, phoneNumber } = req.body ?? {};
    if (
      ![numberId, phoneNumber].every(
        (value) =>
      ...
      return res
        .status(400)
        .json({ msg: 'Number ID and phone number are required (maximum 512 characters)' });
    }
    const values = { numberId: numberId.trim(), phoneNumber: phoneNumber.trim() };
    if (req.method === 'POST') {
      const [row] = await db
        .insert(whatsappConnectNumber)
        .values({ ...values, whatsappConnectId: parent.id })
        .returning();
      return res.status(201).json({ data: row });
    }
    const [row] = await db
      .update(whatsappConnectNumber)
...
export async function handleApiMapping(req: Request, res: Response) {
  if (!validParams(req, res)) return;
  try {
    const [parent] = await db
      .select({ id: salesforceConnect.id })
    ...
    if (!VALID_MAPPING_TYPES.includes(apiMappingType)) {
      return res
        .status(400)
        .json({ msg: `API mapping type must be one of: ${VALID_MAPPING_TYPES.join(', ')}` });
    }
    ...
    if (req.method === 'POST') {
      const [row] = await db
        .insert(ApiMapping)
        .values({ ...values, salesforceConnectId: parent.id })
        .returning();
      return res.status(201).json({ data: row });
    }
    const [row] = await db
