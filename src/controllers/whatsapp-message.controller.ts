...
export async function sendWhatsappMessage(req: Request, res: Response) {
  ...
  if (typeof to !== 'string' || !to.trim() || typeof body !== 'string' || !body.trim()) {
    return res
      .status(400)
      .json({ msg: "'to' (recipient number) and 'body' (message text) are required" });
  }
  ...
  try {
    const [connect] = await db
      ...
      .where(eq(whatsappConnect.tenantId, tenant.tenantId));
    if (!connect) {
    ...
    const condition = numberId
      ? and(numbersScope, eq(whatsappConnectNumber.numberId, String(numberId)))
      : numbersScope;
